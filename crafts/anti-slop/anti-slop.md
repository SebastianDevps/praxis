...
The five tells that survive every rewrite. Full named-tell audit → `prose-tells` skill.

- **"Not just X, but Y."** State the point directly.
- **The rule of three.** Use the number of items the facts have, not three.
...
- **Bold on every proper noun.** Bold only what the reader must not miss.

Stripping tells is half the job. A text with the tells removed and nothing put back reads sterile,
which is its own tell — have an opinion, vary the sentence length, be specific.
