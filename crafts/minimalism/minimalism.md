...
- Before deleting, ask why it might have been written this way — performance, a platform constraint, a historical reason. Check `git blame` for the context the code no longer carries.
- Deletion over addition. When two approaches exist, pick the one that removes more lines.
- Simplification that requires editing a test to pass changed behaviour, not just shape. Revert it, or argue the behaviour change on its own terms. (`evidence-discipline` catches the neighbouring case — an assertion *removed* rather than a value *edited*.)
- **Line-count pressure stops at test code.** Never factor shared setup out of tests to remove lines — prefer descriptive, duplicated phrasing so each test is readable without reading another.
